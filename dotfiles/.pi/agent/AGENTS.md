...
- Do not write long comments. Add comments only when necessary to clear up confusion, and keep them as brief as possible. Do not comment on obvious code. Public doc comments are allowed, but do not state the obvious; if the function is self-describing, keep the description very short. Assume readers can see the function signature, surrounding patterns, package APIs, and other doc comments. Avoid repeating the same information across comments.
